...
variable "efs_access_point_uid" {
  type        = number
  description = "The uid number to associate with the EFS access point" // sonarqube 1000
  default     = 1000
}

variable "efs_access_point_gid" {
  type        = number
  description = "The gid number to associate with the EFS access point" // sonarqube 1000
  default     = 1000
}
...
variable "alb_ingress_allow_cidrs" {
  type        = list(string)
  description = "A list of cidrs to allow inbound into sonarqube."
  default     = null
}
...
variable "sonarqube_controller_port" {
  type    = number
  default = 9000
}

variable "sonarqube_controller_cpu" {
  type    = number
  default = 2048
}

variable "sonarqube_controller_memory" {
  type    = number
  default = 4096
}

variable "sonarqube_controller_task_log_retention_days" {
  type    = number
  default = 30
}

variable "sonarqube_controller_subnet_ids" {
  type        = list(string)
  description = "A list of subnets for the sonarqube controller fargate service (required)"
  default     = null
}

variable "sonarqube_controller_task_role_arn" {
  type        = string
  description = "An custom task role to use for the sonarqube controller (optional)"
  default     = null
}
